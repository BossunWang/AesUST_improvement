...
def extract_target_data(content, style):
    ...
    for f in local_features:
        ...
        # x = (patches, patch_heigth, patch_width, channles)
...
class Model(object):
    def __init__(self, args, content, style, stylized, hist_sim):
        ...
        weight_of_part1 = 0.5
        self.local_sim = weight_of_part1 * (self.local_sim1 / len(LOCAL_STYLE_LAYERS)) + (1 - weight_of_part1) * (
                    self.local_sim2 / len(LOCAL_STYLE_LAYERS))
        ...
        for i in range(len(CONTENT_LAYERS)):
            ...
            target_content = tf.reshape(target_content_norm,
                                        [sem_target.shape[1] * sem_target.shape[2] * sem_target.shape[3], -1])
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            self.cf = self.content_sim.eval()
            self.ge = self.global_sim.eval()
            self.lp = self.local_sim.eval()
...
def main():
    ...
    add_arg('--save_ext', default='.jpg', help='The extension name of the output image')
    add_arg('--output_log', default='log.txt', type=str)
    ...
    stylized_dir = Path(args.stylized)
    ...
    for content_path in tqdm(content_paths):
        for style_path in style_paths:
            stylized_path = stylized_dir / '{:s}_stylized_{:s}{:s}'.format(
                content_path.stem, style_path.stem, args.save_ext)

            content = skimage.io.imread(content_path)
            style = skimage.io.imread(style_path)
            stylized = skimage.io.imread(stylized_path)
    ...
    total_cf_mean = total_cf / total_count
    total_ge_mean = total_ge / total_count
    total_lp_mean = total_lp / total_count
    print('content fidelity:%f, global effect:%f, local patterns:%f.' % (total_cf_mean, total_ge_mean, total_lp_mean))

    fo = open(args.output_log, "w")
    fo.write(
        'content fidelity:%f, global effect:%f, local patterns:%f.' % (total_cf_mean, total_ge_mean, total_lp_mean))
    fo.close()
