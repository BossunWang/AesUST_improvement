...
def main():
    ...
    add_arg('--img_size', type=int, default=512)
    ...
    for content_path in tqdm(content_paths):
        for style_path in style_paths:
            ...
            content = skimage.transform.resize(content, (args.img_size, args.img_size))
            stylized = skimage.transform.resize(stylized, (args.img_size, args.img_size))
            style = skimage.transform.resize(style, (args.img_size, args.img_size))
    ...
    total_lp = np.array(total_lp)

    total_cf_mean = np.mean(total_cf)
    total_ge_mean = np.mean(total_ge)
    total_lp_mean = np.mean(total_lp)
    ...
    fo.write('content fidelity:%f, global effect:%f, local patterns:%f.' % (total_cf_mean, total_ge_mean, total_lp_mean))
    fo.close()
