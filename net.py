...
class Net(nn.Module):
    def __init__(self, content_encoder, vgg, decoder, discriminator, use_patch=False, patch_discriminator=None, patch_size=96, stride=48, top_k=8):
        ...
        content_enc_3 = nn.Sequential(*content_enc_layers[11:18])  # relu2_1 -> relu3_1
        content_enc_4 = nn.Sequential(*content_enc_layers[18:31])  # relu3_1 -> relu4_1
        content_enc_5 = nn.Sequential(*content_enc_layers[31:44])  # relu4_1 -> relu5_1
        self.content_encoder = nn.ModuleList([content_enc_1, content_enc_2, content_enc_3, content_enc_4, content_enc_5])

        vgg_enc_layers = list(vgg.children())
        vgg_enc_1 = nn.Sequential(*vgg_enc_layers[:4])  # input -> relu1_1
        vgg_enc_2 = nn.Sequential(*vgg_enc_layers[4:11])  # relu1_1 -> relu2_1
        vgg_enc_3 = nn.Sequential(*vgg_enc_layers[11:18])  # relu2_1 -> relu3_1
        vgg_enc_4 = nn.Sequential(*vgg_enc_layers[18:31])  # relu3_1 -> relu4_1
        vgg_enc_5 = nn.Sequential(*vgg_enc_layers[31:44])  # relu4_1 -> relu5_1
        self.vgg = nn.ModuleList([vgg_enc_1, vgg_enc_2, vgg_enc_3, vgg_enc_4, vgg_enc_5])
    ...
    def forward(self, content, style, aesthetic=False):
        ...
        style_vgg_feats = self.encode_with_intermediate(self.vgg, style)

        if aesthetic:
            aesthetic_s_feats, _ = self.discriminator(style)
            stylized = self.transform(content_feats[3], style_vgg_feats[3]
                                      , content_feats[4], style_vgg_feats[4]
                                      , aesthetic_s_feats)
        else:
            stylized = self.transform(content_feats[3], style_vgg_feats[3]
                                      , content_feats[4], style_vgg_feats[4])
        ...
        g_t_content_feats = self.encode_with_intermediate(self.content_encoder, g_t)
        g_t_vgg_feats = self.encode_with_intermediate(self.vgg, g_t)
        content_vgg_feats = self.encode_with_intermediate(self.vgg, content)

        # content loss
        loss_c = self.calc_content_loss(g_t_vgg_feats[3], content_vgg_feats[3], norm=True) \
                 + self.calc_content_loss(g_t_vgg_feats[4], content_vgg_feats[4], norm=True)
        # style loss
        loss_s = self.calc_style_loss(g_t_vgg_feats[0], style_vgg_feats[0])
        for i in range(1, 5):
            loss_s += self.calc_style_loss(g_t_vgg_feats[i], style_vgg_feats[i])
        ...
        if aesthetic:   # other losses in stage II
            ...
            Igt = self.decoder(self.transform(g_t_content_feats[3], style_vgg_feats[3]
                                              , g_t_content_feats[4], style_vgg_feats[4]
                                              , aesthetic_g_t_feats))
            l_identity1 = self.calc_content_loss(Igt, g_t.detach())

            Fgt_vgg = self.encode_with_intermediate(self.vgg, Igt)
            l_identity2 = self.calc_content_loss(Fgt_vgg[0], g_t_vgg_feats[0])
            for i in range(1, 5):
                l_identity2 += self.calc_content_loss(Fgt_vgg[i], g_t_vgg_feats[i])
            ...
            # identity loss
            Icc = self.decoder(self.transform(content_feats[3], content_feats[3]
                                              , content_feats[4], content_feats[4]))
            Iss = self.decoder(self.transform(style_vgg_feats[3], style_vgg_feats[3]
                                              , style_vgg_feats[4], style_vgg_feats[4]))

            l_identity1 = self.calc_content_loss(Icc, content) \
                          + self.calc_content_loss(Iss, style)

            Fcc_vgg = self.encode_with_intermediate(self.vgg, Icc)
            Fss_vgg = self.encode_with_intermediate(self.vgg, Iss)
            l_identity2 = self.calc_content_loss(Fcc_vgg[0], content_vgg_feats[0]) \
                          + self.calc_content_loss(Fss_vgg[0], style_vgg_feats[0])
            for i in range(1, 5):
                l_identity2 += self.calc_content_loss(Fcc_vgg[i], content_vgg_feats[i]) \
                               + self.calc_content_loss(Fss_vgg[i], style_vgg_feats[i])
            loss_aesthetic = 0
